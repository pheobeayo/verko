    ...
    >
      <div className="w-[90%] mx-auto py-6">
        <div className="flex items-center justify-between h-16">
          {/* Logo */}
          <Link href={logoHref} className="outline-none">
            <VerkoWordMark size={32} />
          </Link>

          {/* Desktop nav */}
          {!isConnected && <nav className="hidden md:flex items-center gap-6">
            {navLinks.map((l) => (
              <Link
                key={l.href}
                href={l.href}
          ...
          {isConnected ? (
            ...
            <button
              onClick={() => open()}
              className="hidden md:inline-flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold transition-colors duration-200"
              style={{
