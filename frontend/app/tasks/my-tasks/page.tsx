...
export default function MyTasksPage() {
  ...
  // Submission status reads for worker tasks 
  ...
  // Notifications — polls every 30s, toasts on status change
  ...
  return (
    <div className="flex flex-col gap-6 px-4 sm:px-8 py-6 max-w-5xl mx-auto">
      ...
      <div className="flex items-start justify-between">
        ...
        {totalEarned > 0n && (
          <div className="px-4 py-2 rounded-xl border border-[var(--border)] bg-[var(--brown-50)]">
        ...
        {([
          ...
          >
            ...
            <span className={`px-1.5 py-0.5 rounded-full text-[10px] font-bold ${
              activeTab === tab.key ? "bg-[var(--brown-400)] text-[var(--cream-100)]" : "bg-[var(--border)] text-[var(--text-muted)]"
            }`}>
              {tab.count}
            </span>
          </button>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center gap-4 py-16 text-center">
          ...
          <div>
            <p className="font-bold text-sm text-[var(--text-heading)] mb-1" style={{ fontFamily: "var(--font-nunito),sans-serif" }}>
            ...
            <p className="text-xs text-[var(--text-muted)]" style={{ fontFamily: "var(--font-roboto),sans-serif" }}>
              {activeTab === "worker" ? "Browse the marketplace to find work." : "Post a task to get work done."}
            </p>
          </div>
          <button
            onClick={() => router.push(activeTab === "worker" ? "/tasks" : "/tasks/post")}
            className="px-5 py-2 rounded-xl text-sm font-bold bg-[var(--brown-500)] text-[var(--cream-100)] hover:bg-[var(--brown-600)] transition-colors"
            style={{ fontFamily: "var(--font-nunito),sans-serif" }}
          >
        ...
        <div className="flex flex-col gap-4">
          {activeTab === "worker" ? (
            ...
            posterTasks.map((task) => (
              <TaskCard key={task.id.toString()} task={task} onView={setSelectedTask} />
            ))
          )}
        </div>
      )}
